...
import com.jianchen.rentme.activity.myprojects.ProjectCompleteDetailActivity;
    ...
    public void onEvent(ProjectChangeEvent event) {
        if (event.getType() == 2) {
            ...
            for (int i = 0; i < services.size(); i++ ) {
                if (event.getResult().getPr_id() == services.get(i).getPr_id()) {
                    services.get(i).setPr_stts(2);
                    notifyDataSetChanged();
                    break;
                }
            }
